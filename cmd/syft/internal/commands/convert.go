...
func Convert(app clio.Application) *cobra.Command {
	...
	return app.SetupCommand(&cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return RunConvert(cmd.Context(), opts, args[0])
		},
	}, opts)
}
...
func RunConvert(ctx context.Context, opts *ConvertOptions, userInput string) error {
	log.Warn("convert is an experimental feature, run `syft convert -h` for help")

	// a temp dir is only needed when STDIN is a pipe and has to be spilled to disk; it is created lazily on
	// first use (under the user's TMPDIR), so a file input never touches it. Mirrors syft.CreateSBOM, which
	// also honors a TempDir already attached to the context.
	td := tmpdir.FromContext(ctx)
	if td == nil {
		_, td = tmpdir.Root(ctx, "syft-convert")
		defer func() {
			_ = td.Cleanup()
		}()
	}
...
// spilledReader is a seekable view over a spill buffer; closing it releases the buffer's memory and spill file.
