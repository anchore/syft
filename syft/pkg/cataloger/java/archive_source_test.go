...
func Test_archiveContentsCataloger_agreesWithTheArchiveReadingCataloger(t *testing.T) {
	// for the same archive, the contents-reading cataloger must report the same packages with the same
	// identity as the one reading the archive file; any divergence shows up as an SBOM diff
	entries := jarEntries()
	...
	// the archive-reading cataloger copies the archive to a temp dir first, which the contents-reading
	// one does not
	...
	// the contents-reading cataloger, over the same entries laid out as a filesystem, with the traversal
	// the archive cataloger task would have placed on the context
	contentsDir := t.TempDir()
	explode(t, contentsDir, entries)
	contentsResolver, err := fileresolver.NewFromDirectory(contentsDir, "")
	require.NoError(t, err)

	// the traversal names the archive exactly as the resolver did for the archive-reading cataloger, so
	// both sides derive identity from the same string and the comparison is about where the entries came
	// from rather than how the test spelled a path
...
func Test_archiveContentsCataloger_reportsNothingForAnArchiveThatIsNotJava(t *testing.T) {
	// a .zip is an archive and its contents are not a jar's; the archive-reading cataloger draws that
	// line with its globs and this one draws it with the same list
	dir := t.TempDir()
...
func Test_archiveContentsCataloger_noManifestIsNoPackage(t *testing.T) {
	// unchanged from the archive-reading cataloger, and the whole coverage argument: a jar with no
	// META-INF/MANIFEST.MF yields nothing today either, so requiring the contents to be traversed takes
	// away less than it appears to. The maven metadata here is never consulted - without a main package
...
// archive-reading cataloger parses and to lay out the filesystem the contents-reading one reads. The
...
// explode lays the same entries out as files, what the archive cataloger task hands the sub-pipeline
// once it has extracted an archive.
