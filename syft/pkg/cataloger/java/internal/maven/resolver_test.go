...
func Test_resolveProperty(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			resolved := r.ResolveProperty(context.Background(), ptr(test.property), &test.pom)
...
func Test_mavenResolverLocal(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			got := r.ResolveProperty(context.Background(), &test.expression, pom)
	...
	tests := []struct {
	...
	}{
		{
			...
			validate: func(t *testing.T, r *Resolver, pom *Project) {
				...
				got := r.ResolveProperty(ctx, ptr("${commons-exec_subversion}"), pom)
