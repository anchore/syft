...
func (r *Resolver) FindManifests(ctx context.Context, dependency any, df bool, triplet string, parent *pkg.VcpkgManifest) ([]ManifestNode, error) {
	var name string
	var version string
	defaultFeatures := df
	var features []any
	...
	if len(vcpkg.Dependencies) != 0 {
		for _, dep := range vcpkg.Dependencies {
			if !r.depResolved(dep) {
				childManNodes, err := r.FindManifests(ctx, dep, df, triplet, childManifest)
				if err != nil {
...
func (r *Resolver) depRegistry(name string) *pkg.VcpkgRegistryEntry {
...
func (r *Resolver) findManifestFromReg(reg *pkg.VcpkgRegistryEntry, name, version string) (*Vcpkg, error) {
	if reg == nil {
...
func (r *Resolver) getManifestFromGitRepo(repo *git.Repository, head, name, ver string) (*Vcpkg, error) {
...
func (r *Resolver) getManifestFromFilesystem(path, baseline, name, ver string) (*Vcpkg, error) {
	if path == "" {
