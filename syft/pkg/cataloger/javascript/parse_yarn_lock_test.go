...
func TestParseYarnBerryLockWithDuplicates(t *testing.T) {
	// Yarn Berry (v2+) lockfiles can resolve a single package to multiple versions when
	// transitive dependencies require different ranges; the parser must keep every (name, version)
	// tuple. See https://github.com/anchore/syft/issues/4691.
	var expectedRelationships []artifact.Relationship
