...
func getTestAndHybridDeps(mod *extendedBuildInfo, mainPkg *pkg.Package) (map[string]bool, map[string]bool) {
	cfg := &packages.Config{
		Mode: packages.NeedName,
		Dir:  ".",
	}
	// dirPkgs all the packages at the surface of the current dir
	dirPkgs, errLoad := packages.Load(cfg, ".")
	// testPkgs are those who are ascertained to be test-scope packages
	// which are acquired by ```go list -f '{{.TestImports }}' ./``` command in cmd
	testPkgs := make(map[string]bool)
	// testPkgs are those who are ascertained to be the test&normal-scope packages
	// which are acquired by ```go list -f '{{.XTestImports }}' ./``` command in cmd
	hybridPkgs := make(map[string]bool)
	if errLoad != nil {
		return testPkgs, hybridPkgs
	}
	for _, dirPkg := range dirPkgs {
		// if the test binary resides at the right position
		// that means it's generated by 'go test .' and hasn't been moved
		// FIXME: but the way to determine this needs to be enhanced because its a bit hard-coded
		if strings.HasPrefix(mod.Path, dirPkg.ID) {
			PkgInfo, _ := build.ImportDir(".", build.IgnoreVendor)
			for _, testPkg := range PkgInfo.TestImports {
				testModule := strings.Split(testPkg, "/")
				if len(testModule) > 3 {
					testPkg = strings.Join(testModule[:3], "/")
				}
				if !strings.EqualFold(mainPkg.Name, testPkg) {
					testPkgs[testPkg] = true
				}
			}

			for _, hybridPkg := range PkgInfo.XTestImports {
				testModule := strings.Split(hybridPkg, "/")
				if len(testModule) > 3 {
					hybridPkg = strings.Join(testModule[:3], "/")
				}
				if !strings.EqualFold(mainPkg.Name, hybridPkg) {
					hybridPkgs[hybridPkg] = true
				}
			}
			break
		}
...
func createModuleTestRelationships(main pkg.Package, deps []pkg.Package, testPkgs map[string]bool, hybridPkgs map[string]bool) []artifact.Relationship {
	var relationships []artifact.Relationship

	for _, dep := range deps {
		if testPkgs[dep.Name] || hybridPkgs[dep.Name] {
			relationships = append(relationships, artifact.Relationship{
				From: dep,
				To:   main,
				Type: artifact.DependencyOfRelationship,
