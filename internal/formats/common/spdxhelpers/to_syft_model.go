...
func ToSyftModel(doc *spdx.Document2_2) (*sbom.SBOM, error) {
	...
	if doc.CreationInfo != nil {
		src = extractSourceFromNamespace(doc.CreationInfo.DocumentNamespace)
	}
...
// NOTE(jonas): this is a hack, SPDX doesn't seem to support a way
// to describe what the SBOM is about: image, directory for example,
// so I am trying to parse it from namespace. Syft generated SBOMs
// have a type encoded in this field, so this approach works well
// for SBOMs generated by Syft.
func extractSourceFromNamespace(ns string) (src source.Metadata) {
	u, err := url.Parse(ns)
	if err != nil {
		return
	}
	...
	for _, p := range parts {
		switch p {
		case "file":
			src.Scheme = source.FileScheme
			return
		case "image":
			src.Scheme = source.ImageScheme
			return
		case "dir":
			src.Scheme = source.DirectoryScheme
			return
		}
	}
