...
func RegenerateCapabilities(yamlPath string, repoRoot string) (*Statistics, error) {
	...
	updated, orphans, mergeStats := mergeDiscoveredWithExisting(discovered, customCatalogerMetadata, customCatalogerPackageTypes, binaryClassifierGlobs, allCatalogers, existing)
...
func NewCatalogerMerger(registry *CatalogerRegistry, enrichment *EnrichmentData, existing *capabilities.Document) *CatalogerMerger {
...
func (m *CatalogerMerger) addNewGenericCatalogers() {
	for catalogerName, disc := range m.registry.DiscoveredCatalogers() {
		...
		m.updated.Catalogers = append(m.updated.Catalogers, createTemplateEntry(disc, info))
...
func (m *CatalogerMerger) processGenericCataloger(existingEntry *capabilities.CatalogerEntry, disc DiscoveredCataloger, info *capabilities.CatalogerInfo) {
	entry, catalogerOrphans, newParsers := updateEntry(existingEntry, disc, info)
	m.updated.Catalogers = append(m.updated.Catalogers, entry)
...
func mergeDiscoveredWithExisting(discovered map[string]DiscoveredCataloger, customMetadata map[string][]string, customPackageTypes map[string][]string, binaryClassifierGlobs []string, allCatalogers []capabilities.CatalogerInfo, existing *capabilities.Document) (*capabilities.Document, []orphanInfo, *mergeStatistics) {
	...
	merger := NewCatalogerMerger(registry, enrichment, existing)
	return merger.Merge()
}

func updateEntry(existing *capabilities.CatalogerEntry, discovered DiscoveredCataloger, info *capabilities.CatalogerInfo) (capabilities.CatalogerEntry, []orphanInfo, []string) {
	updated := *existing
...
func updateParsers(existingParsers []capabilities.Parser, discoveredParsers []DiscoveredParser, catalogerName string) ([]capabilities.Parser, []orphanInfo, []string) {
	...
	for _, discParser := range discoveredParsers {
		...
		if existingParser == nil {
			// new parser - create with template capabilities
...
func createTemplateEntry(disc DiscoveredCataloger, info *capabilities.CatalogerInfo) capabilities.CatalogerEntry {
	...
	case "custom":
		// custom cataloger with template capabilities
		entry.Capabilities = createTemplateCapabilities()
	}
...
func createTemplateCapabilities() map[capabilities.EnrichmentMode]*capabilities.Capability {
	falseBool := false
	return map[capabilities.EnrichmentMode]*capabilities.Capability{
		capabilities.OfflineMode: {
			License: &falseBool,
			Dependencies: &capabilities.DependencyCapabilities{
				Reach:    []string{}, // empty array indicates TODO
				Topology: "",
				Kinds:    []string{},
			},
			PackageManager: &capabilities.PackageManagerCapabilities{
				...
				PackageIntegrityHash: &falseBool,
			},
		},
	}
}
