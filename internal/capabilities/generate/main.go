...
func hasEmptyCapabilities(caps map[capabilities.EnrichmentMode]*capabilities.Capability) bool {
	if len(caps) == 0 {
		return true
	}
	...
	for _, cap := range caps {
		...
		// check if this is a template entry (empty arrays)
		if cap.Dependencies != nil && cap.Dependencies.Reach != nil && len(cap.Dependencies.Reach) == 0 &&
			cap.Dependencies.Topology == "" &&
			(cap.License == nil || !*cap.License) &&
			(cap.PackageManager == nil || cap.PackageManager.Files == nil ||
				(cap.PackageManager.Files.Listing == nil || !*cap.PackageManager.Files.Listing) &&
					(cap.PackageManager.Files.Digests == nil || !*cap.PackageManager.Files.Digests)) &&
			(cap.PackageManager == nil || cap.PackageManager.PackageIntegrityHash == nil || !*cap.PackageManager.PackageIntegrityHash) {
			return true
		}
	}
