...
func processSPDXLicense(result LicenseList) map[string]string {
	...
	// third pass where we:
	// 1, make variations of the license ID to be overwritten
	// 2. overwrite licenses that were deprecated with supported equivalent ones, selected
	// manually. Ref: https://github.com/anchore/syft/issues/950
	for id, l := range overwrites {
		multipleIDs := buildLicensePermutations(id)
		for _, mID := range multipleIDs {
			if _, exists := overwrites[mID]; !exists {
				overwrites[mID] = l
			}
		}
	}

	for id, o := range overwrites {
		licenseIDs[id] = o
	}
